...
export default ({ state, navigation }) => {
    ...
    return (
        <TabArea>
            ...
            <TabItemCenter activeOpacity={1} onPress={ () => { goTo('Appointments') } }>
                <TodayIcon width="32" height="32" fill="#4EADBE" />
            </TabItemCenter>
            <TabItem onPress={ () => { goTo('Favorites') } } >
                <Favorite style={{opacity: state.index === 3 ? 1 : 0.5}} width="24" height="24" fill="#FFFFFF" />
            </TabItem> 
            <TabItem onPress={ () => { goTo('Profile') } }>
                { user.avatar != '' ?
                    <AvatarIcon source={ { uri: user.avatar } }  style={ { opacity: state.index === 4 ? 1 : 0.5 } } />
                    :
